...
namespace BackEgyVision.Controllers
{
    ...
    public class HomeController : Microsoft.AspNetCore.Mvc.Controller
    {
        ...
        {
            ViewData["type"] = httpContextAccessor.HttpContext.Session.GetString("Role");
            return View();
        }
        ...
        public IActionResult HomeAboutUs()
        {
            try
            {
                IHomeAboutUsService service = new HomeAboutUsService();
                HomeAboutUsVM model =service.Search(new HomeAboutUsVM()).FirstOrDefault();
                if(model == null) {
                    model = new HomeAboutUsVM();
                }
            ...
            {
                return View(new HomeAboutUsVM());
            }
        }
        [HttpPost]
        public IActionResult UpdateHomeAboutUs(HomeAboutUsVM vm)
        {
            try
            {
                IHomeAboutUsService service = new HomeAboutUsService();

                if (vm.HomeAboutUsId > 0)
                {
                    service.Update(vm);
                }
                else
                {
                    vm.TimeInsert = DateTime.Now;
                    service.Insert(vm);
                }
